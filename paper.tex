...
\begin{figure}[h]
    \centering
    \includegraphics[width=3.3 in]{airbox}
...
RSA blind signature\cite{cryptoNote} is used in our research. The data provider chooses a random number $r$ as a blinding factor. $e$ is the broker's public key. To prevent the message $m$ from being known by the broker, the data provider sends blinding message $c$ to the broker instead of $m$.
...
To create a Product Contract, the broker publishes the address of the data provider, subscription fee, brokerage fee, threshold of consent votes of refunding, the channel root and endpoint of MAM channel, and expected number of streaming data.
